...
const FinancialAnalysisPage = () => {
    ...
    // Dynamic Header Logic Based on Savings
    let summarySubtitle = "AI-generated report based on your monthly data.";
    let riskProfile = "Moderate Risk (Balanced Growth)";
    
    if (netSavings <= 0) {
        summarySubtitle = "Alert: You are currently operating at a deficit or zero savings. Focus on capital preservation.";
        riskProfile = "Very Low Risk (Capital Preservation)";
    } else if (netSavings < 300) {
        summarySubtitle = "Solid foundation. It's time to start automating your wealth accumulation.";
        riskProfile = "Moderate Risk (Core Diversification)";
    } else {
        summarySubtitle = "Excellent capital efficiency! You are primed for aggressive portfolio expansion.";
        riskProfile = "High Risk (Aggressive Growth)";
    }
        ...
        <div className="flex h-screen h-[100dvh] overflow-hidden bg-cf-bg">
            ...
            <main className="flex-1 flex flex-col h-screen h-[100dvh] overflow-y-auto w-full min-w-0">
                ...
                <div className="p-4 sm:p-6 md:p-8 max-w-4xl mx-auto w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 sm:mb-8 pb-4 gap-3">
                        <div>
                            <h1 className="text-display-md text-cf-on-surface mb-1 sm:mb-2">Financial Summary & Recommendations</h1>
                            <p className={`text-xs sm:text-sm ${netSavings <= 0 ? 'text-cf-error font-semibold' : 'text-cf-on-muted'}`}>
                                {summarySubtitle}
                            </p>
                        </div>
                    </div>
                    ...
                    {(!budget || budget === 0) && (!expenses || expenses.length === 0) ? (
                        <div className="bg-cf-secondary/10 rounded-xl sm:rounded-2xl p-4 sm:p-6 flex items-start gap-3 sm:gap-4 text-cf-secondary">
                            <AlertCircle className="shrink-0" size={20} />
                            <div>
                                <h3 className="font-display font-bold text-base sm:text-lg mb-1">No Budget or Expenses Defined</h3>
                                <p className="text-xs sm:text-sm text-cf-on-muted">Please go to the Expenses page to enter your monthly budget or expenses to see your analysis.</p>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-6 sm:space-y-10">
                            {/* Monthly Overview */}
                            <section>
                                <h2 className="font-display text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-cf-on-surface">Monthly Overview:</h2>
                                <div className="bg-cf-surface-high rounded-xl sm:rounded-2xl p-4 sm:p-6">
                                    <ul className="space-y-3 sm:space-y-4 text-sm">
                                        <li className="flex gap-2 items-center flex-wrap">
                                            <span className="text-label-sm text-cf-on-muted w-28 sm:w-36">Budget</span> 
                                            <span className="text-cf-on-surface font-display font-bold text-base sm:text-lg" style={{ fontVariantNumeric: 'tabular-nums' }}>₹{(Number(budget) || 0).toFixed(2)}</span>
                                        </li>
                                        <li className="flex gap-2 items-center flex-wrap">
                                            <span className="text-label-sm text-cf-on-muted w-28 sm:w-36">Total Expenses</span> 
                                            <span className="text-cf-on-surface font-display font-bold text-base sm:text-lg" style={{ fontVariantNumeric: 'tabular-nums' }}>₹{totalExpenses.toFixed(2)}</span>
                                        </li>
                                        <li className="flex gap-2 items-center flex-wrap">
                                            <span className="text-label-sm text-cf-on-muted w-28 sm:w-36">Net Savings</span> 
                                            <span className={`font-display font-bold text-base sm:text-lg ${netSavings >= 0 ? 'text-cf-tertiary' : 'text-cf-error'}`} style={{ fontVariantNumeric: 'tabular-nums' }}>
                            ...
                            {/* Expense Breakdown */}
                            <section>
                                <h2 className="font-display text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-cf-on-surface">Analysis & Breakdown:</h2>
                                <div className="grid grid-cols-1 gap-4 sm:gap-6">
                                    <div className="bg-cf-surface-high rounded-xl sm:rounded-2xl p-4 sm:p-6 relative overflow-hidden">
                                        <h3 className="font-display text-base sm:text-lg font-bold mb-3 sm:mb-4 text-cf-on-surface flex items-center gap-2">
                                            <span className="w-2 h-5 sm:h-6 bg-cf-error rounded-full inline-block shadow-glow-error"></span>
                                        ...
                                        {Object.keys(categoryTotals).length === 0 ? (
                                            ...
                                            <ul className="space-y-2 sm:space-y-3">
                                                {Object.entries(categoryTotals).map(([cat, total]) => (
                                                    <li key={cat} className="flex gap-2 items-center text-xs sm:text-sm">
                                                        <span className="text-cf-on-muted w-28 sm:w-32">• {cat}:</span> 
                            ...
                            <section>
                                <h2 className="font-display text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-cf-on-surface">Investment Recommendations ({riskProfile}):</h2>
                                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-cf-on-surface mb-4 sm:mb-6">
                            ...
                            <section className="pb-8 sm:pb-12">
                                <h2 className="font-display text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-cf-on-surface">Financial Tips:</h2>
                                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-cf-on-surface mb-4 sm:mb-6 bg-cf-surface-high rounded-xl sm:rounded-2xl p-4 sm:p-6">
                                    {generateTips().map((tip, index) => (
                                        <p key={index} className="flex gap-2">
                                            <span className="text-cf-primary shrink-0">•</span> 
